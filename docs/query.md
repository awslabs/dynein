# `dy query` command
In dynein, you can use `dy query` command to retrieve items that match the condition specified.
...
| `BETWEEN A AND B` or `BETWEEN A B` | a value of the sort key is between `A` and `B` (inclusive at both side) |
| `>=`                               | a value of the sort key is greater than or equal to`A`                  |
| `BEGINS_WITH A`                    | a value of the sort key has a prefix A                                  |

The keywords `BETWEEN`, `AND`, and `BEGINS_WITH` do not matter in their cases.
