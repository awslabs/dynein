...
Options:
  -s, --sort-key <SORT_KEY_EXPRESSION>
          Additional Sort Key condition which will be converted to KeyConditionExpression. Valid syntax: ['= 12', '> 12', '>= 12', '< 12', '<= 12', 'between 10 and 99', 'begins_with myVal"]

      --consistent-read
          Strong consistent read - to make sure retrieve the most up-to-date data. By default (false), eventual consistent reads would occur. https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/HowItWorks.ReadConsistency.html
  ...
  -a, --attributes <ATTRIBUTES>
          Attributes to show, separated by commas, which is mapped to ProjectionExpression (e.g. --attributes name,address,age). Note that primary key(s) are always included in results regardless of what you've passed to --attributes

      --keys-only
          Show only Primary Key(s)

  -d, --descending
          Results of query are always sorted by the sort key value. By default, the sort order is ascending. Specify --descending to traverse descending order
