...
pub async fn export(cx: app::Context, given_attributes: Option<String>, keys_only: bool, output_file: String, format: Option<String>) -> Result<(), IOError> {
    ...
    match format_str {
        Some("csv")    => { if !keys_only && given_attributes.is_none() {
            attributes = overwrite_attributes_or_exit(&cx, &ts).await.expect("failed to overwrite attributes based on a scanned item");
        } },
        None | Some(_) => { if keys_only || given_attributes.is_some() {
            app::bye(1, "You can use --keys-only and --attributes only with CSV format.")
        } },
    }
    ...
    let mut tmp_output_file: fs::File = fs::OpenOptions::new().create(true).append(true).open(tmp_output_filename)?;
    ...
    loop {
        ...
        let scan_output: ScanOutput = data::scan_api(cx.clone(),
            None  /* index */,
        ...
        let items = scan_output.items.expect("Scan result items should be 'Some' even if no item returned.");
        match format_str {
            None | Some("json") => {
            ...
            Some("csv") => {
                let s = data::convert_items_to_csv_lines(&items, &ts, &attrs_to_append(&ts, &attributes), keys_only);
                tmp_output_file.write_all(s.as_bytes())?;
            },
        ...
        debug!("scan_output.last_evaluated_key is: {:?}", &scan_output.last_evaluated_key);
    ...
    match format_str {
        ...
        Some("csv") => csv_finish(f, &tmp_output_filename, &ts, attrs_to_append(&ts, &attributes), keys_only)?.write_all(b"\n")?,
        Some(o) => panic!("Invalid output format is given: {}", o),
    };
...
pub async fn import(cx: app::Context, input_file: String, format: Option<String>) -> Result<(), batch::DyneinBatchError> {
    ...
    let input_string: String = if Path::new(&input_file).exists() {
        ...
        error!("Couldn't find the input file '{}'.", &input_file); std::process::exit(1);
    };
    ...
    match format_str {
        ...
        Some("jsonl") => {
            ...
            let array_of_json_obj: StreamDeserializer<'_, StrRead, JsonValue> = Deserializer::from_str(&input_string).into_iter::<JsonValue>();
            // list_of_jsons contains deserialize results. Filter them and get only valid items.
            let array_of_valid_json_obj: Vec<JsonValue> = array_of_json_obj.filter_map(Result::ok).collect();
...
async fn overwrite_attributes_or_exit(cx: &app::Context, ts: &app::TableSchema) -> Result<Option<String>, IOError> {
...
async fn suggest_attributes(cx: &app::Context, ts: &app::TableSchema) -> Vec<SuggestedAttribute> {
    ...
    let items = data::scan_api(cx.clone(),
                    None /* index */, false /* consistent_read */, &None /* attributes */,
    ...
    let non_key_attributes = items[0].iter().filter(|(attr, _)|
                               !primary_keys.iter().any(|key| Some(attr.to_owned()) == key.as_ref() )
...
fn json_finish(mut f: fs::File, tmp_output_filename: &str) -> Result<fs::File, IOError> {
    ...
    let len = contents.len(); contents.truncate(len - 1); // remove last ","
...
fn csv_finish(mut f: fs::File, tmp_output_filename: &str,
              ts: &app::TableSchema, attributes_to_append: Option<Vec<String>>, keys_only: bool)
...
fn build_csv_header(ts: &app::TableSchema, attributes_to_append: Option<Vec<String>>, keys_only: bool) -> String {
...
async fn write_array_of_jsons_with_chunked_25(cx: app::Context, array_of_json_obj: Vec<JsonValue>) -> Result<(), batch::DyneinBatchError> {
